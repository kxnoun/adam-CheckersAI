...
class CState:
    ...
    def get_p(self, row, col): # get a piece
        return self.board[row][col]
    
    def set_p(self, row, col, piece): # set a piece
...
def cutoff_check(state: CState, depth, max_depth):
    return depth >= max_depth or is_terminal(state)

def max_value(state: CState, alpha, beta, depth, max_depth, trans_table):
    if cutoff_check(state, depth, max_depth):
    ...
    v = -float('inf')
    successors = sorted(state.gen_successor('r'), key=lambda x: evaluate(x, 'r'), reverse=True)
    for successor in successors:
        v = max(v, min_value(successor, alpha, beta, depth + 1, max_depth, trans_table))
        if v >= beta:
            trans_table[state] = v
            return v
        alpha = max(alpha, v)
...
def min_value(state: CState, alpha, beta, depth, max_depth, trans_table):
    if cutoff_check(state, depth, max_depth):
    ...
    v = float('inf')
    successors = sorted(state.gen_successor('b'), key=lambda x: evaluate(x, 'b'))
    for successor in successors:
        v = min(v, max_value(successor, alpha, beta, depth + 1, max_depth, trans_table))
        if v <= alpha:
            trans_table[state] = v
            return v
        beta = min(beta, v)
...
def alpha_beta_search(state: CState, depth, max_depth, maximizing_player, trans_table):
    best_value = -float('inf') if maximizing_player else float('inf')
    best_state = None

    successors = sorted(state.gen_successor('r' if maximizing_player else 'b'),
                        key=lambda x: evaluate(x, 'r' if maximizing_player else 'b'),
                        reverse=maximizing_player)

    for successor in successors:
        if maximizing_player:
            value = min_value(successor, -float('inf'), float('inf'), depth + 1, max_depth, trans_table)
            if value > best_value:
                best_value = value
                best_state = successor
        else:
            value = max_value(successor, -float('inf'), float('inf'), depth + 1, max_depth, trans_table)
            if value < best_value:
                best_value = value
                best_state = successor
...
def play(init_state, max_depth):
    ...
    while not is_terminal(curr_state):
        if turn == 'r':
            best_move, _ = alpha_beta_search(curr_state, depth, max_depth, True, trans_table)
            turn = 'b'
        else:
            best_move, _ = alpha_beta_search(curr_state, depth, max_depth, False, trans_table)
            turn = 'r'
...
if __name__ == '__main__':
    ...
    init_board = read_from_file(args.inputfile)
    init_state = CState(init_board)
    x = time.time()
    sol_states = play(init_state, 10)
    print(f"Time: {time.time() - x} seconds")
    write_output(args.outputfile, sol_states)
    print(f"{len(sol_states) - 1} moves")
